...
static void free_socket(struct shapeshifter_obfs4_socket_posix *sock)
{
    if (!sock)
        return;
...
static void shapeshifter_obfs4_posix_request_event(openvpn_vsocket_handle_t handle, openvpn_vsocket_event_set_handle_t event_set, unsigned rwflags)
{
...
static bool shapeshifter_obfs4_posix_update_event(openvpn_vsocket_handle_t handle, void *arg, unsigned rwflags)
{
    shapeshifter_obfs4_log(((struct shapeshifter_obfs4_socket_posix *) handle)->ctx, PLOG_DEBUG, "update-event: %p, %p, %d", handle, arg, rwflags);
...
static unsigned shapeshifter_obfs4_posix_pump(openvpn_vsocket_handle_t handle)
{
    shapeshifter_obfs4_log(((struct shapeshifter_obfs4_socket_posix *) handle)->ctx, PLOG_DEBUG, "pump -> %d", ((struct shapeshifter_obfs4_socket_posix *) handle)->last_rwflags);
...
static ssize_t shapeshifter_obfs4_posix_sendto(openvpn_vsocket_handle_t handle, const void *buf, size_t len, const struct sockaddr *addr, socklen_t addrlen)
{
    ...
    shapeshifter_obfs4_log(((struct shapeshifter_obfs4_socket_posix *) handle)->ctx, PLOG_DEBUG, "sendto(%d) -> %d", (int)len, (int)number_of_characters_sent);
...
static void shapeshifter_obfs4_posix_close(openvpn_vsocket_handle_t handle)
{
